...
export const load = (
  ...
  // Relative to the including file first, then the lib root.
  const locate = (request: string, fromFile: string, at: Location): string => {
    const candidates = isAbsolute(request)
      ? [request]
      : [resolvePath(dirname(fromFile), request), resolvePath(libRoot, request)]
...
export const libRootFor = (root: string): string => join(root, 'lib')
