...
| `momoed` | the editor - an explorer beside a text pane, toggled away for width, and text modes `edit.com` never had | §54's buffer, and directory enumeration - §38 put that out of scope and names the DTA collision behind it |
...
- **A text buffer.** §54 - a line as a chain of chunks, so an edit is bounded by
  a chunk rather than by the document and a line has no length limit. `momoed` is
  going ahead, and this is the part of it that a program can exercise before the
  editor around it exists. It needs nothing from the language: three shapes were
  drafted and run on the target before one was chosen, and the one chosen was not
  the one the design started from.
...
- **The screen library, in part.** 2026-09-07. §43, now in `DESIGN.md`, and the
  ...
  stopped carrying their own copies - the five that set mode 13h by hand and four
  more that only saved and restored. A text mode's geometry is read back from the
  BIOS rather than taken from the table, which is what tells 43 rows from 50 after
  an identical pair of calls. Still designed and not built: the properties query
  with its fallback chain, aspect ratio, the interleaved, planar and banked
  layouts, and windowing - every one of which serves a program blocked on a mouse.
...
- **`block`.** 2026-09-07. §47, now in `DESIGN.md`, and the record is DECISIONS
  ...
  startup sequence, so §16 took a segment register as a third address source
  instead. Going to check whether it already could found that `= _ds` compiled and
  emitted a read from a label nothing defines - a miscompile that had been under
  §35 since it landed, along with a second one in the same shape that needed no
  `far` region at all.
...
What is left for those three is a **library**, not a language change: something
over that word that makes a back buffer and an allocator ordinary code. That is
`std/block.momo`, and DESIGN §47 has it.
...
| | | |
|---|---|---|
| back buffer, far arena, backing stores | §35 only - no DOS call | **open**, and `std/block.momo` (§47) is the library over it |
| runtime `_hsize` | nothing; specified in §13 | **open** - the same `PSP:0x0002` word |
| arena and zone allocators | nothing - libraries, no compiler change | **open**; §47 is deliberately not one |
...
**Designed, not built.** The first thing `momoed` needs, and the only part of it
designed so far. A line is a chain of chunks, the chunks are deliberately not
kept full, and every edit is bounded by a chunk rather than by the document.

The name is the one thing here that is not settled. `motext` follows the
convention and nothing else has been proposed.
...
group line[maxLines] {            // a line names a chain, never owns text
  u16 head
  u16 length
}

view u8[maxChunks * chunkSize] text = _heap[0]
```

A chunk's bytes live at `chunk * chunkSize`, a line's text is the chunks on its
...
by documented 8086 timings. Measured from the emitted assembly on 2026-09-09.

That one figure decides the whole design, and it decides it against the obvious
answer. **A gap buffer is the right structure on a machine with a fast block
move, and this machine has none**: moving a 16 KB gap is about 0.6 seconds, and a
`u16` view halves it to 0.3. So the buffer has to be one that never moves bulk
text, and every candidate below is judged on that.

### Three shapes were drafted, and two were measured out

Each was drafted as a working program, run under DOSBox and measured. The drafts
were throwaway and are not in the repository; the figures are from 2026-09-09.
...
text, which is what recommended it - and it moves *line offsets* instead, of
which there are far more than there are bytes in a line. Roughly 18 ms a
keystroke at 500 lines and 35 ms at 1,000, growing with the file.

**Fixed slots lost on the line cap**, and the cap cannot be lifted by widening
the slot: 255 characters across 400 lines is 102,000 bytes against a 64 KB
segment. A line long enough to be worth supporting cannot have a slot of its own.

**Chunks turned out to be faster than fixed slots as well as unbounded**, which
was not the expected result. The byte shift is bounded by the chunk rather than
by the line width, so the common case gets cheaper at the same time as the
uncommon one becomes possible.

### Redraw does not distinguish any of them, and the naive interface is a trap
...
hoisted per line, so **redraw is not an argument for either shape**.

What it is instead is a rule, and it applies to the chunk chain equally: **the
chain is walked once per line, never once per character.**

### Far memory is the wrong home for this, and §16 says why

Everything here lives in `_heap` through a `view` (§17). A `far` region past the
...
- **§38 requires a DOS buffer in our own segment.** `DS = CS` cannot be
  ...
  copying out - through the 180-cycles-a-byte loop above, twice per file.
...
Removing the table took 254 bytes off the image and cost no cycles. The rule is
the distinction rather than either half of it: **a table of row addresses earns
its place when the stride is imposed - 320, or 80 - and not when the stride is
ours to pick, because then it is picked as a power of two and §26 does the work.**

The same thing pays again in the undo log, where `% maxUndo` emits
`and ax, 31` rather than a `div`.
...
line cannot go below half occupancy however long the session runs. Measured: 54
characters in 4 chunks became 24 characters in 3, where without the merge the
count stayed at 4.
...
**The merge is also load-bearing for correctness**, which is not obvious and was
not designed. An emptied chunk left on a chain takes `used` below zero on the
next deletion, and the line then reports 255 bytes of whatever the heap holds
after it. Two things follow, and both are rules rather than observations: a
deletion must step over a *run* of empty chunks rather than one, and it must
refuse a chunk holding nothing rather than trusting the merge to have removed it.

### Undo records edits, not storage

An entry is an operation, a line, a column and a character - six bytes - and
nothing in it knows what a chunk is. So the log is the same log over any of the
three buffers above, and reversal goes back through the ordinary operations:
insert against delete, split against join.

**That settles a claim this section inherited and disproves it.** The piece table
was argued for partly on undo being nearly free over a piece list. It is equally
free over all of them, so undo is not an argument for any buffer shape.
...
- **The chain is walked once per line, never per character.** It holds for
  rendering and for any operation that reads a run.
...
- **Recording is suspended while an undo runs, and while a file loads.** A
  ...
  in is not an edit anybody wants to step back through.
...
indirection and §38's file handles.

One friction is worth naming without being a request. `lineSeek` answers three
things and a routine returns one, so the answers travel in globals. It reads
acceptably and nothing else in the repository has wanted multiple returns, so it
is recorded here rather than proposed.
...
The undo log is six bytes an entry, so a 32-entry window is 192 bytes.

### Scope of a first build

In: chunk chains with split and merge, line records, insert, delete, split, join,
render, the free list, and the undo log with the three operations above.

Out: redo; undo coalescing, so that a run of typed characters is one step rather
