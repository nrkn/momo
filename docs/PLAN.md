...
- **Is `$` Momo's string terminator, or DOS's?** No stage of the compiler knows
  ...
  that and §20 says why. §53 is built, and its length spine is the counted form
  for static data, so the positions can be prototyped now rather than argued.
...
- **Nested arrays.** 2026-09-23. §53, now in `DESIGN.md`, and the record is
  ...
  reach the image - a constant index reads neither table. Two things came into
  the first build that the design had left out, both because momoed's menus
  needed them: `addr( menu[i] )`, without which a child could reach no routine
  taking an address, and the length spine. `of` over one loads the child's
  address once an iteration, decided by the parser from syntax alone.
...
- **Text past the segment.** 2026-09-13. §58, now in `DESIGN.md`, and the record
  ...
  The image came down ten kilobytes while capacity went up between three and
  eight times - 8,192 chunks, 4,000 lines and 512 undo entries against 2,400,
  900 and 64.

  **A chunk is a paragraph**, which is the one thing the plan did not anticipate:
  a single far region cannot be indexed past 64 KB, so chunk `c` is the paragraph
  `c` past the base and no index holds more than a chunk. The plan also watched
  the wrong half - what refused `momoed.momo` was 920 lines against 900, not its
  26 KB against 38.
- **Selection and the clipboard.** 2026-09-13. §61, now in `DESIGN.md`, with the
  selection itself in §56 and a group of edits in §54; the record is DECISIONS
  §61. Shift with any motion extends, `^A`, `^C`, `^X` and `^V`, and `^C`/`^X`
  with nothing selected take the line. A mark and a point rather than a range,
  which is also the shape multiple cursors want - recorded in §55 as a direction,
  along with the finding that it needs no change to any of this.

  **It is what spent the segment.** Under a kilobyte of the 64 KB is unclaimed
  afterwards, which is why the undo log is still sixty-four entries and why a cut
  of more than one line reports *too big to undo* rather than half restoring. The
  fix is one byte per character instead of seven and it is a different shape, not
  a bigger number - though §58, immediately after, made room for a bigger number
  as well and the log is now 512 entries.
...
- **A text buffer.** 2026-09-09. §54, now in `DESIGN.md`, and the record is
  ...
  limit. Three shapes were drafted as working programs and run on the target
  before one was chosen, and the one chosen was not the one this started from: the
  piece table lost on its line index rather than on its pieces. It needed nothing
  from the language and includes nothing, not even `std`.
...
- **§53's `menu[i]`.** Listed among the things an array-typed access may be
  ...
  not this mechanism. Withdrawn when §53 was built: the runtime case got
  `addr( menu[i] )` instead, which needed nothing from this section.
