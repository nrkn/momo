...
The first draft of this section had the author finding tooling too much work for
the return, which makes it a cost-benefit call that happened to come out the same
way for twenty years. His own account is considerably less comfortable than that,
and it is worth setting down in full because every part of it does work.

**He knew.** Not in hindsight - throughout. He was aware the whole time that
tooling was the thing standing between fixating on engines and actually building
the games he had ideas for. He knew that rewriting the same engine repeatedly was
the symptom. He knew that good editors were the cure. **And he already knew how to
build them, because he was building them for clients the entire time.** Diagnosis,
prescription and capability, all present, for two decades, and the thing was not
done - which he describes as looking almost pathological in hindsight.

That is a different claim from a blind spot, and a stranger one. A blind spot is
something you cannot see. This was in view continuously and did not move.

**One qualification, and it is not a small one.** *Small games finished.* This is
not a record of nothing shipping - the 1 kB roguelikes shipped, one of them
submitted as a numbered js1k demo; a LORD homage in 1017 bytes shipped with the
commit "Done - submitted - bang etc"; RANGER DOWN was a complete js13k entry in
2018 and was re-released on itch.io seven years later; `tennis` runs on real
hardware. What never eventuated was the larger ambitions, consistently, and those
are exactly the ones whose content would not fit in a source file.
...
never missing; it was spent where someone was paying for it, which is also why the
bias note above applies here - most of that work is not catalogued, so this
document under-reports the strongest half of the evidence.

### A byte budget is a substitute for tooling, and that is why the small ones shipped

Put the two halves together and the pattern stops being a mystery about
motivation.

**Under a hard size limit the content is small enough to write in code.** A 1 kB
roguelike has no sprite sheet worth an editor - its map is generated by a hash and
its glyphs are characters. A 13 KB game can carry a hand-authored tile set as
literals. At that scale the missing editor costs nothing, because there is nothing
for it to edit. So the constraint that looks like the hard part is quietly
removing the dependency that actually stops him.

Above that scale the content outgrows the source file, an editor becomes load
bearing, and the project stops. **The finished games are the ones small enough not
to need the thing he never built** - which is why the archive reads as a long run
of shipped miniatures beside a long run of abandoned engines, and why those two
runs are the same fact seen twice.

**RANGER DOWN is the whole pattern inside one title.** It was finished once, in
2018, as a js13k entry under a 13 KB budget - the only complete version of it that
exists. Since then it has been restarted repeatedly with no budget: a from-scratch
re-engineering in 2025, a sequel parked pending a jam theme, a map generator
extracted far enough to be handed to a Unity collaborator, and a 2023 treatment
whose central mechanic is that collectibles **upgrade the renderer** - unlocking
1bpp to 2bpp colour, per-tile palettes, animation states, pixel movement, and
silence to beeps to a sequencer to samples. That is an extraordinary design idea
and it is also, read from here, a game about acquiring the tools to make the game.
Of the 2025 attempt the catalogue notes that the one finished file is a settings
object recovering the 2018 constants.
