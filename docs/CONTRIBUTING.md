...
**`mlolayer` is the same claim for a screen made of several layouts**, and it has
two demos rather than one. A window is its own tree at its own size and a screen
is the list of them with an offset apiece, so overlapping needs no floating and
no element that remembers where it was put. `s6demo` is a System 6 desktop, where
a window is simply placed; `pmdemo` is Program Manager, where the group windows
are MDI children placed against a client area read back out of the layout beneath
them. `mlolayer` holds every box in both against the study with the offsets
folded into x and y, which is what checks that derivation rather than only the
geometry. The packing that puts the icons in rows is `shared/lib/moflow.momo`,
above the engine for the reason its header gives.
...
427 tier-1 assertions, 40 e2e programs, all green. `npm test` prints the tier 1
